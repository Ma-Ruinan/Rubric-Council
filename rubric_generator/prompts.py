...
问题文件：{_path(task_dir / '问题描述.txt', project_dir)}
内部素材清单与提取结果：{_path(material_manifest, project_dir)}
临时计算目录：{_path(scratch_dir, project_dir)}

权威顺序：当前提示和用户要求 > 问题描述与源素材 > 经核验的版本/时点事实 > Skill。Skill 只作方法参考。
...
5. 每组权重各自合计 100。ID 命名空间固定：要求 REQ、事实锚点 K、完成率 T、覆盖 C、准确 A、格式 F、结构 S、幻觉 H；不得用 A 同时表示锚点和准确率原子。每个原子只使用 BIN/RATIO/COUNT/CLAIM-RATIO 之一，并写出可计算公式、证据和溯源。CLAIM-RATIO 必须定义主张切分方式、核验全集/抽样规则，并用结构化字段 `empty_set_value`（仅 0 或 1）与 `empty_set_reason` 定义无适用主张的处理；其他函数的这两个字段固定为 null 和空字符串。
...
authoring record：{_path(authoring_record, project_dir)}
...
任务完成率与五项质量均须有至少一个题目原生原子；不得用通用填充项创造新义务。每组原子权重各自合计 100。所有 mandatory 要求必须被状态规则真实计分，而不是只在引用 ID 中挂名。CLAIM-RATIO 的 `empty_set_value` 与 `empty_set_reason` 必须和 record 完全一致；自然分母不得替换成无依据硬阈值。锚点用 K 前缀，准确率原子用 A 前缀。错误规则只保留平均分会掩盖的、会改变结论的实质错误；单个普通错误或后来失效的链接不得触发严重封顶。
...
题目目录：{_path(task_dir, project_dir)}
内部素材清单：{_path(material_manifest, project_dir)}
当前 record：{_path(current_record, project_dir)}
当前 Rubric 规格：{_path(current_spec, project_dir)}
审核意见：{_path(review_file, project_dir)}
...
题目目录：{_path(task_dir, project_dir)}
内部素材清单：{_path(material_manifest, project_dir)}
修订后 record：{_path(revised_record, project_dir)}
上一版规格：{_path(previous_spec, project_dir)}
审核意见：{_path(review_file, project_dir)}

只落实 blocking 与修订后 record。保持固定六项接口和自包含要求；再次检查 mandatory 要求真实计分、CLAIM-RATIO 空集合、阈值来源、K/A 命名空间及严重封顶的实质性条件。只输出：
BEGIN_RUBRIC_SPEC_JSON
{json.dumps(rubric_spec_schema_example(), ensure_ascii=False, indent=2)}
END_RUBRIC_SPEC_JSON
"""
...
def review(project_dir: Path, task_dir: Path, material_manifest: Path, authoring_record: Path, spec_file: Path, rubric_file: Path, round_number: int, previous_review: Path | None = None, arbitration_file: Path | None = None, final_verification: bool = False) -> str:
    previous = _path(previous_review, project_dir) if previous_review else "无"
    arbitration = _path(arbitration_file, project_dir) if arbitration_file else "无"
...
内部素材清单：{_path(material_manifest, project_dir)}
authoring record：{_path(authoring_record, project_dir)}
Rubric 规格：{_path(spec_file, project_dir)}
程序渲染 Rubric：{_path(rubric_file, project_dir)}
上一轮审核：{previous}
绑定裁决：{arbitration}
...
以下属于 blocking，不得降为 suggestion：mandatory 要求只挂引用 ID 而状态规则未观察；CLAIM-RATIO 缺空集合/全集规则；无来源的硬阈值；record 与 spec 的指标归属、权重、状态函数或引用关系存在会改变评分的语义冲突；内部构建产物泄漏；外部来源使用裸域名、猜测地址或虚假 `verified` 状态；不可访问来源无替代证据却单独支撑关键锚点；以当前链接失效惩罚历史交付；来源不能直接支持锚点；单个普通错误触发与实质性不相称的严重封顶。record 与 spec 的同义改写、解释详略或非评分措辞差异不是 blocking。审核时必须引用并检查原子的精确状态规则，不得根据标题、锚点说明或自己的推断替它补足。
...
Rubric 规格：{_path(spec_file, project_dir)}
程序渲染 Rubric：{_path(rubric_file, project_dir)}
Reviewer 原始输出：{_path(raw_review_file, project_dir)}
...
def arbitrate(project_dir: Path, task_dir: Path, material_manifest: Path, current_record: Path, current_spec: Path, record_history: list[Path], spec_history: list[Path], review_files: list[Path]) -> str:
    records = "\n".join(f"- {_path(path, project_dir)}" for path in record_history)
    specs = "\n".join(f"- {_path(path, project_dir)}" for path in spec_history)
    reviews = "\n".join(f"- {_path(path, project_dir)}" for path in review_files)
    return f"""对一轮修改后仍未解决的结构化 Rubric 分歧作一次绑定裁决。

题目目录：{_path(task_dir, project_dir)}
内部素材清单：{_path(material_manifest, project_dir)}
最新 record：{_path(current_record, project_dir)}
最新规格：{_path(current_spec, project_dir)}
record 历史：
{records}
规格历史：
{specs}
