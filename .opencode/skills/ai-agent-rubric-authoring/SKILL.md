...
14. **Make ratios total functions.** `CLAIM-RATIO` defines claim segmentation, the complete audit universe or justified deterministic sample, and records its empty-set result in structured `empty_set_value` and `empty_set_reason` fields. Prefer natural denominators to invented pass thresholds.
